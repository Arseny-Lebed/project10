...
> **Примечание:** В предоставленном коде реализован генератор синтетических данных, имитирующий типичные фенологические кривые NDVI (рост, пик, спад), что позволяет проверить работоспособность решения без подключения к API GEE.
...
scikit-learn>=1.0.0
matplotlib>=3.5.0
earthengine-api>=0.1.300  # Опционально, для работы с реальными данными GEE
```
